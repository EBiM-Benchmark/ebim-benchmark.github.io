...
// UNLISTED /zh/feedback-registered.html and /zh/feedback-phase2.html
// questionnaires — all plain noindex zh pages with no hreflang/toggle and out
// of the sitemap (see contactSuccessChecks / registerSuccessChecks /
// openDaySuccessChecks / openDayMunichSuccessChecks / computeApplyChecks /
// computeSuccessChecks / feedbackRegisteredChecks / feedbackPhase2Checks below).
//
...
// redirect (src/zh/contact-success.njk). It is NOT one of the nine localized
...
// The hidden /zh/ UNLISTED Questionnaire B page (src/zh/feedback-phase2.njk),
// modelled on feedbackRegisteredChecks — NO i18nKey, so NO hreflang, NO navbar
// language toggle, OUT of the sitemap; a plain noindex zh page (noindex from the
// zhNoindex computed). Its only cross-locale link is the page-local EN switch
// back to ../feedback-phase2.html, and it submits lang=zh. Field/option/
// condition/required parity with the EN page is scripts/verify-questionnaire.mjs's job.
function feedbackPhase2Checks() {
  const file = "zh/feedback-phase2.html";
  const checks = [];
  const add = (name, ok, msg = "") => checks.push({ name, ok, msg });
  ...
  add(
    "canonical=self",
    html.includes(
      '<link rel="canonical" href="https://ebim-benchmark.github.io/zh/feedback-phase2.html" />',
    ),
    "expected self canonical to /zh/feedback-phase2.html",
  );
  ...
  add(
    "page-local EN switch",
    html.includes('<a class="q-lang-opt" lang="en" href="../feedback-phase2.html">EN</a>'),
    'expected <a class="q-lang-opt" lang="en" href="../feedback-phase2.html">EN</a>',
  );
  add(
    'hidden lang="zh"',
    html.includes('<input type="hidden" name="lang" value="zh" />'),
...
function main() {
  ...
  // ── hidden /zh/ UNLISTED Questionnaire B page ──
  console.log(BOLD("• zh/feedback-phase2.html  (HIDDEN UNLISTED)"));
  for (const c of feedbackPhase2Checks()) {
    console.log(`    ${c.ok ? GREEN("PASS") : RED("FAIL")}  ${c.name}`);
    if (!c.ok) {
      allOk = false;
      fails.push(`zh/feedback-phase2.html — ${c.name}: ${c.msg}`);
    }
  }
  console.log("");
  ...
  siteAdd(
    "sitemap excludes /feedback-phase2.html + /zh/ (hidden unlisted)",
    !sitemap.includes("feedback-phase2.html"),
    "feedback-phase2.html (EN or /zh/) must not appear in sitemap.xml",
  );
