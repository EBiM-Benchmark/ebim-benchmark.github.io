...
- [x] Competition timeline (**revised schedule locked 2026-07-22** — the Jun 29 release slipped, a **developer preview is now live**, and the post-preview schedule was re-cut; the Simulation Release row reads "Preview live" rather than a date. `event.json` now holds `startDate`/`eventPublishStartDate` `2026-07-06` and `endDate` `2026-09-12` under `EventScheduled`, so the PR #93 date gate is **open** and index/competition emit the `Event` again): Simulation Release **(preview live)** → Simulation End **Aug 15, 2026 (AoE)** → Results Announced **Aug 18** → Phase II two-window (team hands-on bench testing **Aug 19–31**, local venue time; organizer-run bench testing & evaluation **Sep 1 – Sep 12**) → Phase II submission deadline / competition end **Sep 12, 2026 (AoE)**. Submission deadlines are **AoE**; bench-testing windows run on **local venue time**. Repository submissions have been open since the developer preview; workshop date & Final Results TBD
