...
**Measured now** (Chromium, self-hosted Inter; widths are `documentElement.clientWidth`, i.e. excluding the ~15px scrollbar): logo 152.7 + items 841.3 = **row 994.0** in a **1052** box → **+58.0px headroom**. Per-item: Home 86.4 · Competition 131.1 · Workshop 116.6 · **Open Day 96.6** · Register 86.0 · Partners 87.8 · FAQ 55.8 · Contact 83.2 · 中文 97.9. Nothing folds at any width. The row's box is `min(viewport, 1100) − 48`, so it needs a **≥1042px viewport** to fit — that one formula reproduces every threshold below. `/zh/` is far roomier (CJK labels are compact): row **868.0** → **+184px headroom**, fitting fully at ≥916px, with the same failure mode below — its toggle only goes off-screen under ~879px (measured: still on-screen at 880, off-screen at 875).
