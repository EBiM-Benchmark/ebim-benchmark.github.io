...
**The one formula.** Every threshold here derives from: **`box = min(clientWidth, 1100) − 48`**, and the row must fit the box. All widths below are `documentElement.clientWidth` — **not** window width. A real window is ~15px wider (the classic scrollbar), so a **1024px laptop window is clientWidth ≈1009**. Mixing the two units is what produced a wrong "1024px" claim in three earlier revisions of this file.
...
**Measured now** (Chromium, self-hosted Inter, this machine's caret font; `.7rem` padding): EN logo 152.7 + items 802.9 = **row 955.6** in a **1052** box → **+96.4px headroom** at the cap; fits fully down to **clientWidth 1004**. `/zh/` is far roomier (CJK labels are compact): **row 829.7** → **+222.3px headroom**, fitting down to clientWidth 878. Nothing folds at any width ≥769 on either locale.
...
> **⚠️ Residual, pre-existing: the row has no responsive strategy between 769px and ~1004px (clientWidth).** The drawer only engages at ≤768px, so between those widths the row does not fit and the rightmost items (the language toggle first) run past the viewport with **no horizontal scroll to reach them**. Measured thresholds, EN, post-trim (clientWidth): **≥1004** fully clean; **980–1003** the row eats into `.nav-inner`'s 24px right padding but everything stays visible; **<980** the toggle goes **off-screen**. A real 1024px window is clientWidth ≈1009 → **clear, by ~29px** — the trim moved it out of the boundary zone it sat in at `.85rem`. `/zh/` has the same failure mode far narrower: off-screen only below clientWidth ~841.
>
...
| Raise the drawer breakpoint 768 → ~1004 | Removes the residual band entirely | Edits `style.css`; changes UX for 769–1004 visitors. The *principled* fix for the band |
