...
English is the primary locale. A **Simplified-Chinese `/zh/` locale** was added in **Phase 1b**: the landing + competition pages were **published in Phase 1d** once their translation passed native review; the workshop + contact pages shipped as unpublished machine-draft previews (noindex, no hreflang/sitemap/toggle) in **Phase 2b** and were **published in Phase 2c** after native review — so all four `/zh/` pages are now live. Publish state is **per page** (see the gate below). Structured-data and shared-chrome strings are factored out so a locale is added without touching the EN templates:
...
`sitemap.xml` is rendered from `src/sitemap.njk` (per-page gated on `zhPublished`): the 4 EN URLs (home, competition, workshop, contact) plus a `/zh/` URL for each **published** page — as of Phase 2c all four are published, so **8 URLs** (4 EN + 4 `/zh/`) with `xhtml:link` hreflang alternates on all four pairs. It is referenced from `robots.txt`.
...
- [x] Organizers: OC (10), Advisory (4), Support (17); headshot photos wired for 30/31 cards (1 remains as initials avatar)
...
### Still needed
- [ ] Real headshot for the remaining initials-avatar card (Sonya Yang); the other 30 organizer cards now use photos
