...
- **`affil-consistency.py`** — run on any PR that adds, removes or edits a person on the site. It compares each speaker's agenda row against their own speaker card, same page and locale; it cannot detect a wrong-but-consistent affiliation, and makes no cross-page assertion by design (see the `/zh/` mixing rule above).
