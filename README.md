...
| **Register** | `register.html` | Team registration — native Web3Forms form (team of 1–6, task selection, testbed ranking, consent; + `register-success.html` no-JS fallback). Own access key, separate from Contact. **The testbed-ranking fieldset still lists Pittsburgh and still ranks four sites even though Pittsburgh was withdrawn as a testbed in PR #120 — deliberately.** Dropping the select changes the Web3Forms export shape: the `Testbed Rank Pittsburgh` column would vanish for new rows while existing rows keep it, and the registration analysis reads that column. Decide it deliberately (probably keep the column and default it), not as a rider on a content PR. A team that ranks Pittsburgh has it removed and their remaining preferences move up |
