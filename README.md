...
`node scripts/verify-zh.mjs` (alias `npm run verify:zh`, or `npm run verify:all` to run every stage) checks the `/zh/` locale against the same build, with every gated assertion reading `site.zhPublished` so it is correct in either state. Always: each `/zh/` page is `<html lang="zh-Hans">`, has a self-referential `/zh/` canonical, links all four localized targets relative under `/zh/`, and contains translated CJK text. Published (the current state for all ten pages): no `noindex`, the reciprocal `en` / `zh-Hans` / `x-default` hreflang cluster, `/zh/` present in `sitemap.xml` (20 URLs total) with `hreflang` on all ten EN + `/zh/` pairs, and the navbar **language toggle** rendering with 中文 active + an "EN" link to the EN counterpart. Unpublished: `noindex`, no `hreflang` anywhere, `/zh/` absent from the sitemap, and no toggle. It also covers the hidden `/zh/contact-success.html`, `/zh/register-success.html`, `/zh/open-day-success.html` and `/zh/open-day-munich-success.html` utility pages, plus the unlisted `/zh/compute-apply.html` + `/zh/compute-success.html` and `/zh/feedback-registered.html` — `<html lang="zh-Hans">`, a single `noindex`, no `hreflang`/toggle, out of the sitemap, CJK body, `../` assets (the questionnaire additionally: its page-local EN link and `lang=zh`). Each Open Day check additionally pins that day's zh RSVP redirect at its OWN `/zh/` success page — Hamburg at `/zh/open-day-success.html`, Munich at `/zh/open-day-munich-success.html`. Within a day the EN and zh forms share one access key, so a copy-pasted EN redirect would silently land zh registrants on the English confirmation; across days the keys differ, so a copy-pasted Hamburg redirect on the Munich form would land Munich registrants on the Hamburg page. Both blocks are state-aware and read their own flag. CI runs every harness on every PR.
...
`node scripts/verify-questionnaire.mjs` (alias `npm run verify:questionnaire`) is the questionnaire's EN/zh parity gate. Both questionnaire pages render from one data file, but a template edit could still make them diverge, so it reads the **built** pages and fails if they differ in any form control or question-wrapper tag (only the hidden `lang` value may differ), in the submitted field names, in any choice field's option codes or order, in `data-group` (what makes a checkbox group go out as one `;`-joined field), in any condition, in the hidden values or the close date, or in any inline script. It also checks both against `src/_data/questionnaireA.json` (field list, codes, conditions, and each field's control type), checks that every condition names a real field and code and that conditional questions ship hidden + disabled, and fails if any control ships pre-answered or if the page carries a control the script would send but the check does not model. Because a single-source script edit changes both pages alike, `verify.mjs` also pins the EN page's inline script to its golden.
