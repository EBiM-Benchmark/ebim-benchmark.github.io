...
| **Feedback** *(unlisted)* | `feedback-registered.html` | Questionnaire A for teams that registered but did not file a Phase I submission — own Web3Forms key + hCaptcha. Every question, option code, condition and both languages live in `src/_data/questionnaireA.json`; the EN and `/zh/` pages render from it through shared includes, and `verify-questionnaire.mjs` fails the build if the two built pages would submit anything different. Each checkbox group is sent as one `;`-joined field. The page closes itself at `closesAt` (end of 22 Oct 2026 AoE). URL is emailed only, so it is kept out of nav/footer/sitemap (`noindex`); the two pages link only to each other |
